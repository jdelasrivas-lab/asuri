...
set.seed(5)
outputKM <- geneSurv(seBRCA, time, status, geneName, type = "exprs", verbose = F)
```
...
geneName <- "BRCA1"
set.seed(5)
outputKM.TP53 <- geneSurv(seBRCA, time, status, geneName, type = "risk", verbose = F)
```
...
set.seed(5)
DE_list_genes <- prefilterSAM(seBRCA, groupsVector, verbose = F)
```
