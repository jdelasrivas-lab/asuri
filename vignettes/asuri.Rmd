...
- name: Maria Sanchez-Martin
  affiliation: Bioinformatics and Functional Genomics Group - Cancer Research Centre (CiC-IBMCC, CSIC/USAL) and IBSAL, Salamanca (Spain)
- name: Santiago Bueno-Fortes
  affiliation: Bioinformatics and Functional Genomics Group - Cancer Research Centre (CiC-IBMCC, CSIC/USAL) and IBSAL, Salamanca (Spain)
- name: Natalia Alonso-Moreda
  affiliation: Bioinformatics and Functional Genomics Group - Cancer Research Centre (CiC-IBMCC, CSIC/USAL) and IBSAL, Salamanca (Spain)
...
abstract: |
  ...
  **Key Features**:
    ...
    4. Predict patient risk and classify into risk groups (**predict_PatientRisk**). 
    
    5. Estimate survival for novel query patients (**predict_SurvCurve**).
---
...
Once this dataset is loaded the *seBRCA* object is created. This SummarizedExperiment object contains the normalized gene expression data matrix and phenotypical information about each sample (i.e. clinical physio-pathological data and other phenotypic data).

```{r, echo = TRUE, eval = TRUE} 
library(asuri); library(SummarizedExperiment)
data(seBRCA)
# To extract the normalized expression
mExprs <- assay(seBRCA)
mPheno <- colData(seBRCA)
```

```{r, echo = TRUE, eval = TRUE} 
# To visualize the dimensions of the two extracted objects
dim(mExprs)
dim(mPheno)
```
...
```{r, echo = TRUE, eval = TRUE, fig.show = "hide", results="asis"} 
data(seBRCA)
time <- "time"
status <- "status"
geneName <- "ESR1"
...
set.seed(5)
outputKM <- geneSurv(seBRCA, time, status, geneName, type = "exprs", verbose = FALSE)
```
...
```{r pclassesr1, fig.width=7.5, fig.height=7.5, fig.cap="Patient Class Probability for low exprs (89) and high exprs (111) groups).", out.width = "80%", fig.align = "center", echo=FALSE, results="asis"}
...
```{r boxesr1, fig.cap="Boxplot: high expression vs low expression for ESR1 gene.", fig.width=7.5, fig.height=7.5,  out.width = "80%", fig.align = "center", echo=FALSE, results="asis", results="asis"}
...
```{r kmesr1, fig.cap="Kaplan Meier curves obtained after the assignment of the patients by bootstrap to high or low survival using the selected feature ESR1.", fig.width=7.5, fig.height=7.5, out.width = "80%", fig.align = "center", echo=FALSE, results="asis"}
plotKM(outputKM)
```
...
geneName <- "BRCA1"
set.seed(5)
outputKM.TP53 <- geneSurv(seBRCA, time, status, geneName, type = "risk", verbose = FALSE)
```

```{r kmrisk, fig.cap="Kaplan Meier curves obtained after the assignment of the patients by bootstrap to high or low survival using the estimated risk based on feature BRCA1.", fig.width=7.5, fig.height=7.5, out.width = "80%", fig.align = "center", echo=FALSE, results="asis"}
...
The Hazard Ratio (HR) reported in the analysis is estimated using a univariate Cox Proportional Hazards model. Specifically:

1. **Methodology**: The HR is calculated comparing the "High Risk" group 
   against the "Low Risk" (reference) group, or in the case of expression 
   "High Expr" against the "Low Expr" group. The estimation is performed 
   using the `hazard.ratio` function from the `survcomp` package.
2. **Interpretation**: An $HR > 1$ indicates an increased risk of the event 
   for the high (risk-expr) group, while an $HR < 1$ suggests a protective effect.
3. **Statistical Rigor**: All HR estimates are provided with their 
   corresponding 95% Confidence Intervals (CI) and p-values derived from the 
   Wald test. The proportionality of hazards is an underlying assumption for 
...
The input parameters for the function are the following:
  ...
  - `groupsVector`: A binary vector indicating the group to which each sample is assigned.
...
groupsVector <- colData(seBRCA)$ER.IHC
  
set.seed(5)
DE_list_genes <- prefilterSAM(seBRCA, groupsVector, verbose = FALSE)
```
...
- `seBRCA`: SummarizedExperiment object with the normalized expression data and the phenotypic data in colData.
- `DEgenes`: Vector containing the genes to be used. Expected to be in the same format as the rows of the **assay(seBRCA)**. Usually this vector is the result of running **prefilterSAM()** function.
...
- `vectorSampleID`: Vector containing the sample names ordered as in the **assay(seBRCA)**.
...
DE_list_genes <- ex_prefilterSAM
vectorSampleID <- as.character(rownames(colData(seBRCA)))
vectorGroups <- as.numeric(colData(seBRCA)$ER.IHC)
...
```{r, echo = TRUE, eval = TRUE} 
knitr::kable(head(Pred_ER.IHC$betasTable, 10), 
             caption = "List of genes by Stability.")
```
...
# Training of the multivariate COX model. Provide the SummarizedExperiment 
# object with the normalized expression data and the phenotypic data in colData,
...
set.seed(5)
multivariate_risk_predictor <- patientRisk(seBRCA, geneList, time, status, method = "class.probs")
```
...
- `group.vector` Optional numeric vector specifying predefined risk groups for the patients.
...
num_samples <- 20
set.seed(5)
mExprs_testData <- matrix(rnorm(length(geneList) * num_samples, mean = 10, sd = 3),
...
The input parameters of the function **predict_PatientRisk()** are:
...
The output of the function **predict_PatientRisk()** is a risk plot and a list with two objects:
...
This function provides a plot with the predicted risk score using the multivariate COX model trained by function **patientRisk()** and can be applied also to predict the risk of a single patient given the gene expression profile. The mExprs_testData will contain now a column with the expression level for the list of genes selected.

```{r, echo = TRUE, eval = TRUE, fig.align = "center", results="asis", fig.width=7.5, fig.height=7.5, out.width = "80%" } 
...
risk_prediction_one_patient$risk_score
```
...
## Survival estimation from the COX regression for a test patient: **predict_SurvCurve()**

The function **predict_SurvCurve()** will obtain the expected survival curve for a test patient estimated from the risk score predicted by a multivariate COX regression model and considering the Breslow estimator for the baseline Hazard. The function provides a plot for the expected survival curve.
...
head(cox_pred_training$risk_score)
...
# Brief comparison of ASURI and other related R tools
The ASURI package has several features that make it attractive to clinicians and researchers specialising in molecular cancer, setting it apart from other libraries available in R/Bioconductor. For instance, the BIOSPEAR package [@ternes2018biospear] provides a framework for biomarker selection based on multivariate penalised Cox regression models. BIOSPEAR enables the selection of various Lasso Cox regression variants, such as Adaptive Lasso, Elastic Net, stability selection and group Lasso. Based on previous models, it can also estimate the expected survival probability for individual patients. However, this library identifies gene signatures that cannot be related to the clinical markers or pathophysiological features that clinicians use to characterize diseases, diagnose patients, and decide on treatment plans. This key feature differentiates ASURI from other survival packages. To this end, ASURI introduces a robust pipeline that allows us to discover survival genes related to main clinical parameters. This feature facilitates translating biomarker discovery into clinical-related decisions. ASURI also implements a bootstrapping strategy for multivariate gene selection, which improves the robustness and reproducibility of the gene signatures provided by BIOSPEAR. Finally, ASURI provides methods to stratify the risk curve, optimising the splitting and providing fuzzy classifications of patients at intermediate risk.

A second package related to ASURI is CPSM [@kaur2024cpsm], that enables the identification of a set of biomarkers using univariate or multivariate Lasso Cox regression. These features can then be used to predict the survival probability of individual patients using a Multi-Task Logistic Regression model. This package is more ASURI-like and also provides ways to interact with clinical information. However, as previously mentioned, ASURI incorporates methods to improve the robustness of the selected markers and to correlate them with standard clinical features used by oncologists to characterize tumors. CPSM lacks these characteristics.

A third ASURI-related package is RTNsurvival [@groeneveld2019rtnsurvival]. This package focuses on the multivariate survival analysis of regulons obtained from transcriptional regulatory networks derived by the RTN package. It implements two pipelines to relate regulon activity to patient survival: the first is based on multivariate Cox models, and the second uses Kaplan-Meier curves. Both analyses can be performed using the ASURI package if the regulon activity is provided. Note that the multivariate methods implemented by ASURI are more robust due to the resampling strategy employed. Furthermore, ASURI has a broader focus and can robustly select subsets of marker genes from a large number of gene features. It can also accurately predict prognosis and survival. This goal cannot be achieved using RTNsurvival.

Finally, two Bioconductor packages that are less related to ASURI: survcomp [@schroder2011survcomp] and genefu [@gendoo2016genefu]. The survcomp package is more oriented towards evaluating and comparing survival models. In fact, this library is imported and inserted into the ASURI package to estimate certain performance metrics in survival analyses. Genefu is a package focused in the computation of gene expression-based signatures, especially in breast cancer studies, and is oriented more towards comparison of gene signatures and risk predictors based on these gene signatures. In contrast, ASURI focuses on identifying new molecular targets robustly, accurately predicting risk and stratifying patients into optimal risk groups.
