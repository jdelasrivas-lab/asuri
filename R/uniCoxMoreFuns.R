initcx <- function(y, ic) {
    ...
    kq <- junk$kq
    tq <- junk$tq[seq(1, kq)]
    ddq <- junk$ddq[seq(1, kq)]
    iriskq <- junk$iriskq[seq(1, kq)]
...
findroot <- function(beta1, beta2, lam, x, sx, iriskq, 
                     kq, ddq, ic, del = .001) {
    SMALL <- 1e-6
    ...
    if (sign(f1) * sign(f0n) < 0) {
        val <- binary.search(beta1, 0, lam, x, sx, iriskq, kq, 
                             ddq, ic, del = del)$beta
    }
    if (sign(f2) * sign(f0p) < 0) {
        val <- binary.search(0, beta2, lam, x, sx, iriskq, kq, 
                             ddq, ic, del = del)$beta
    }
    if ((sign(f1) * sign(f0n) > 0) & (sign(f2) * sign(f0p) > 0)) {
        val <- 0
    }
    return(val)
}
...
comp.path <- function(lamlist, betalims, x, sx, iriskq, kq, ddq, ic) {
    ...
    val[1] <- findroot(betalims[1], betalims[2], lamlist[1], x, sx, 
                       iriskq, kq, ddq, ic)
    betahat <- val[1]
    go <- TRUE
    ii <- 1
...
coxscor <- function(x, y, ic, offset = rep(0., length(y))) {
    ...
    # nn=#obs in each risk set, nno=sum(exp(offset)) at each failure time
...
coxvar <- function(x, y, ic, 
                   offset = rep(0., length(y)), coxstuff.obj = NULL) {
    ...
    for (i in seq(2., nf)) {
        oo <- (seq(1., n))[y >= fail.times[i - 1] & y < fail.times[i]]
        sx <- (1 / nno[i]) * 
...
coxscor2 <- function(x, y, ic, offset = rep(0., length(y))) {
    ...
    for (i in (seq(1., nf))) {
        w <- w + s[, i]
        oo <- (seq(1., n))[y >= fail.times[i]]
        r <- rowSums(x[, oo, drop = FALSE] * exp(offset[, oo, drop = FALSE]))
        w <- w - (d[i] / nno[, i]) * r
    }
...
coxstuff2 <- function(x, y, ic, offset = rep(0., length(y))) {
    ...
    for (i in seq(1., nf)) {
        nn[i] <- sum(y >= fail.times[i])
        nno[, i] <- rowSums(exp(offset)[, y >= fail.times[i], drop = FALSE])
    }
    ...
    return(list(fail.times = fail.times, s = s, d = d, 
                dd = dd, nf = nf, nn = nn, nno = nno))
}

coxvar2 <- function(x, y, ic, 
                    offset = rep(0., length(y)), coxstuff.obj = NULL) {
    # version that allows offset to be a matrix
    ...
    for (i in seq(2., nf)) {
        ...
        sx <- (1 / nno[, i]) * 
          (nno[, i - 1] * sx - 
             my.rowSums(x[, oo, drop = FALSE] * exp(offset[, oo])))
        
        s <- (1 / nno[, i]) * 
          (nno[, i - 1] * s - 
             my.rowSums(x2[, oo, drop = FALSE] * exp(offset[, oo])))
